...
public class AddMutationStrategy implements MutationStrategy {
    ...
    public ParametersBundle applyMutation(Rule originalRule, ArrayList<Rule> rules, ParametersBundle parametersBundle) {
        ...
        try {
            ...
            if (statePostcondition == null) {
                ...
                log.debug("No 'State' postcondition found in rule '{}'. Skipping mutation.", originalRule.getRule_name());
            ...
            ArrayList<Fact> actionsClone = (ArrayList<Fact>) mutatedRule.getActions();
            ArrayList<Fact> postconditionsClone = (ArrayList<Fact>) originalRule.getPostconditions();
            ...
            ArrayList<Value> knowledgeContent = extractKnowledgeAndAddStatePostcondition(mutatedRule, postconditionsClone);
            ...
            ArrayList<Fact> messagesToSend = buildMessagesFromPermutations(baseSendFact, permutations, knowledgeContent);

            // For each role in the environment, create mutated send rules and handle arrival mutations
            processRolesAndMutations(originalRule, mutatedRule, messagesToSend, statePostcondition, parametersBundle);

        } catch (Exception e) {
            // Catch-all to ensure we log any unexpected issues
            log.error("Error while adding mutation for rule '{}': ",
    ...
    /**
     * Removes unused variables from the mutated rule by comparing "Rcv" preconditions
     * and "Snd" postconditions from the original rule.
     */
    private void removeUnusedVariables(Rule mutatedRule, Fact rcvPrecondition, Fact sndPostcondition) {
        try {
            ...
            if ((sndVarNames != null && rcvVarNames != null) && (!sndVarNames.isEmpty()
                    && !rcvVarNames.isEmpty())) {
                ...
                if (sndVarNames != null) {
                    for (String varName : sndVarNames) {
                        if (!varName.isEmpty() && variable.getName().equals(varName)) {
                            mutatedRule.getVariables().set(
                                    mutatedRule.getVariables().indexOf(variable), null
                            );
                            break;
                        }
                    }
    ...
    /**
     * Extracts variable names from a specific parameter index of a Fact if that parameter is a Variable.
     */
    ...
    /**
     * Finds the 'State' Fact in postconditions, adds it to the mutatedRule,
     * and returns the PSpecial knowledge content.
     */
    private ArrayList<Value> extractKnowledgeAndAddStatePostcondition(Rule mutatedRule, ArrayList<Fact> postconditionsClone) {
    ...
    /**
     * Creates a base "Snd" Fact using the rule's postcondition Snd as a template.
     * Sets the receiver to "empty".
     */
    ...
    /**
     * Given a base "Snd" Fact with "empty" receiver, build multiple messages using
     * each permutation of knowledge content.
     */
    private ArrayList<Fact> buildMessagesFromPermutations(Fact baseSendFact,
    ...
    /**
     * For each role (agent), create the mutated send rule, then find the matching arrival
     * rules for that receiving agent, and mutate those as well.
     */
    private void processRolesAndMutations(Rule originalRule,
                                          Rule mutatedRule,
        ...
        Value humanAgentValue = (Value) statePostcondition.getParameter(0);
            ...
            for (Fact messageToSend : messagesToSend) {
                for (Value role : parametersBundle.getRoles()) {
                    ...
                    if (!role.getName().replaceAll("[^a-zA-Z0-9]", "")
                        ...
                        mutateArrivalRules(theoryClone2, scenarioMutatedRule, messageToSend, role, parametersBundle);
    ...
    private void addSndActionsForMessage(Rule scenarioMutatedRule, Fact mutatedMessage) {
        ...
        // If content is a Variable, we do not add anything here (original code was empty).
        else if (contentObj instanceof Variable) {
            // No specific action in the original snippet
        }
        // If content is a single Value, just add one "Snd" action
    ...
    /**
     * Finds arrival rules for the specified 'receiverAgent' in the theory and mutates them
     * to handle the new "Rcv" message. Recursively mutates the chain of 'next' rules as well.
     */
    private void mutateArrivalRules(ArrayList<Rule> theoryClone,
    ...
    /**
     * Adds corresponding "Rcv" actions to the arrival rule for the third parameter
     * of the message (PSpecial or Value).
     */
    ...
    /**
     * Rebuilds the first postcondition as a new "State" derived from the preconditions.
     * Removes any old 'Snd' postconditions if found.
     */
    private void rebuildPostState(Rule arrivalRule) {
        ...
        Fact newStateFact = utilityFunctions.buildNewState(arrivalRule.getPreconditions(), preAgentState);
    ...
    private Rule nextRule(ArrayList<Rule> theory, String agentName, int stateIndex) {
        ...
        for (Rule r : theory) {
            ...
            if (statePre != null) {
                Value aName = (Value) statePre.getParameter(0);
                Value sNum  = (Value) statePre.getParameter(1);
                if (aName.getName().equalsIgnoreCase(agentName)
                        && sNum.getName().equals(String.valueOf(stateIndex))) {
                    return r;
                }
            }
        }
