    ...
    public ParametersBundle applyMutation(Rule originalRule, ArrayList<Rule> rules, ParametersBundle parametersBundle) {
        try {
            ...
            for (Rule arrivingRule : nextRules) {
                ...
                parametersBundle = rulesModifier.rulesModifier(theoryClone, true, arrivingRule, parametersBundle);
        ...
        if (postSend != null) {
            ...
            for (Mutants mutant : modifications) {
                ...
                for (Object param : postSend.getParameters()) {
                    ...
                    else if (param instanceof PSpecial pSpecial) {

                        // Loop through all the Values inside the PSpecial
                        for (int i = 0; i < pSpecial.numberOfValues(); i++) {
                            Value innerValue = pSpecial.getValue(i);
            ...
            if (action.getF_name().equals("Snd") || action.getF_name().equals("To")) {
                ...
                for (int i = 0; i < modifications.size(); i++) {
                    ...
                    for (Object parameter : action.getParameters()) {
                        ...
                        if (value.getName().equals(mutant.getNewValue().getName()) && !value.isInKnowledge() && !mutant.getNewValue().isInKnowledge()) {
