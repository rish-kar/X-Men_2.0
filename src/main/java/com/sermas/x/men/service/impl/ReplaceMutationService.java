...
public class ReplaceMutationService {
     ...
     */
    public ParametersBundle replaceMutation(Rule rule, ArrayList<Rule> theory, ParametersBundle parametersBundle) {
...
//        parametersBundle.setTheory(theory);
    ...
    private ArrayList<Value> extractSndParameters(Rule rule) {
        ...
        try {
            ...
            if (postSendOut != null) {
                ...
                else if (sendParameter instanceof Variable) {
                    Variable message = (Variable) sendParameter;
     ...
     */
    private void handleTagReplacements(Rule rule, ArrayList<Rule> theory, ArrayList<Value> parameters,
        ...
        try {
            ...
            for (Mutants mutant : changesToApply) {
                ...
                Rule clonedRule = shouldCloneRule(parametersBundle)
                        ? rule.clone()
                ...
                clonedRule.setTypo(com.sermas.x.men.model.Type.MUTATED);
    ...
    /**
     * Handles the process of replacing submessages within the given rule and theory,
     * based on the parameters extracted from the "Snd" postcondition.
     *
     * @param rule       The rule to mutate.
     * @param theory     The original theory that might be cloned or updated.
     * @param parameters The parameters extracted from the "Snd" postcondition.
     */
    private void handleSubmessageReplacements(Rule rule, ArrayList<Rule> theory, ArrayList<Value> parameters,
                                              ParametersBundle parametersBundle) {
        try {
            ...
            for (String permutation : permutations) {
                ArrayList<Rule> theoryClone = new ArrayList<>(utilityFunctions.cloneModel(parametersBundle));

                // Build a list of Values to send based on the current permutation
                ArrayList<Value> newContent = (ArrayList<Value>) generateArrayofValuetoSend(permutation, parameters);
    ...
    private boolean shouldCloneRule(ParametersBundle parametersBundle) {
        // Clarify the original combined condition: (parametersBundle.getFlags().isCombineAddReplace() && parametersBundle.getFlags().isSwitchFlag()) || parametersBundle.getFlags().isCombineAddReplaceOnly()
        return (parametersBundle.getFlags().isCombineAddReplace() && parametersBundle.getFlags().isSwitchFlag())
     ...
     */
    private void updateActionsWithMutants(Rule clonedRule, Mutants mutant, ParametersBundle parametersBundle) {
            ...
            for (Object param : action.getParameters()) {
                if (param instanceof Value) {
                    ...
                    // Otherwise, check if there's a functional replacement
                        ...
                        Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), paramValue, mutant);
     ...
     * @param mutant     The mutation details (old/new values, etc.).
     */
    private void updateSndPostcondition(Rule clonedRule, Mutants mutant, ParametersBundle parametersBundle) {
        ...
        if (valuesSent instanceof PSpecial) {
            PSpecial pspecial = (PSpecial) valuesSent;
            for (int i = 0; i < pspecial.getGroup().size(); i++) {
                ...
                // Otherwise, check for functional replacement
                    ...
                    Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), oldValue, mutant);
                    if (potentialValue != null) {
        ...
        // If "Snd" is a single Value, do a direct substitution or functional replacement
            ...
            Value sentValue = (Value) valuesSent;
            if (mutant.getOldValue().equals(sentValue)) {
                outPost.getParameters().set(2, mutant.getNewValue());
            } else {
                Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), sentValue, mutant);
                if (potentialValue != null) {
    ...
    /**
     * Generates all permutation strings based on the size of the parameters list (via power set of indices).
     *
    ...
    /**
     * Builds a list of potential Mutants by comparing values from knowledgeList and sentList.
     * A Mutant is created whenever two values have the same non-null tag but different names.
     *
     * @param knowledgeList the list of Value objects representing knowledge in the postcondition
     * @param sentList      the list of Value objects that were sent
     * @return a list of Mutants representing potential replacements
     */
    private ArrayList<Mutants> buildPotentialReplacement(List<Value> knowledgeList, List<Value> sentList) {
     ...
     */
    public void propagationReplace(ArrayList<Rule> theory, Mutants mutants, ParametersBundle parametersBundle) {
        try {
            ...
            if (ruleToBeModified != null && (parametersBundle.getFlags().isCombineAddReplace() && parametersBundle.getFlags().isSwitchFlag() || parametersBundle.getFlags().isCombineAddReplaceOnly())) {
            ...
            while (ruleToBeModified != null) {
                    ...
                    if (valuesReceived instanceof PSpecial) {
                        for (int i = 0; i < ((PSpecial) valuesReceived).getGroup().size(); i++) {
                            ...
                            if (mutants.getOldValue().equals(value)) {
                                ...
                                Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), value, mutants);
                                if (potentialValue != null) {
                        ...
                        for (Object parameter : action.getParameters()) {
                            if (parameter instanceof Value) {
                                if (mutants.getOldValue().equals(parameter)) {
                                    action.getParameters().set(action.getParameters().indexOf(parameter), mutants.getNewValue());
                                } else {
                                    Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), (Value) parameter, mutants);
                                    if (potentialValue != null) {
                                        potentialValue.setTag(((Value) parameter).getTag());
                                        action.getParameters().set(action.getParameters().indexOf(parameter), potentialValue);
                                    }
                                }
                            }
                        }
                ...
                if (postSend != null) {
                    ...
                    if (valuesSent instanceof PSpecial) {
                        ...
                        if (mutants.getOldValue().equals(valuesSent)) {
                            ...
                            Value potentialValue = utilityFunctions.checkFunctionReplacement(parametersBundle.getFunctions(), (Value) valuesSent, mutants);
                            if (potentialValue != null) {
                ...
                if (parametersBundle.getFlags().isCombineAddReplace() && parametersBundle.getFlags().isSwitchFlag() || parametersBundle.getFlags().isCombineAddReplaceOnly()) {
    ...
    private Fact buildMessageToSend(Fact postSendFact, ArrayList<Value> valuesToSend) {
        try {
            ...
            if (originalMessage instanceof PSpecial) {
                PSpecial originalPSpecial = (PSpecial) originalMessage;
            ...
            else if (originalMessage instanceof Variable) {
                Variable originalVariable = (Variable) originalMessage;
                Special originalSpecial = originalVariable.getValues();
                if (originalSpecial instanceof PSpecial) {
                    PSpecial originalPSpecial = (PSpecial) originalSpecial;
                    ArrayList<Value> originalValues = originalPSpecial.getGroup();
                    updateValues(originalValues, valuesToSend);
    ...
    private void recursiveVariable(Variable variable, ArrayList<Value> valuesToUpdate) {
        try {
            ...
            if (variable.getValues() instanceof FSpecial) {
                FSpecial functionalSpecial = (FSpecial) variable.getValues();
                ...
                for (; i < group.size() && !valuesToUpdate.isEmpty(); i++) {
                    ...
                    if (element instanceof Variable) {
                        ...
                        Value originalValue = (Value) element;
            ...
            else if (variable.getValues() instanceof PSpecial) {
                PSpecial positionalSpecial = (PSpecial) variable.getValues();
     ...
     * <p>
     * This method locates a {@link Rule} in the provided {@code theory} list, modifies subsequent
     * rules, adjusts their preconditions and postconditions (e.g., "State", "Rcv", "Snd"),
     * and optionally adds those modified rules back into the theory. It also updates a global
     * {@code collections} structure with the final state of the theory.
     * </p>
     *
     ...
     */
    private void propagationReplaceSubmessages(ArrayList<Rule> theory, ArrayList<Value> contentClone, ParametersBundle parametersBundle) {
        try {
            ...
            if (ruleToModify != null && (parametersBundle.getFlags().isCombineAddReplace()
            ...
            while (ruleToModify != null) {
                ...
                ruleToModify.setTypo(com.sermas.x.men.model.Type.MUTATED);
                ...
                if (preAgentState != null) {
                    ...
                    int parsedStateNumber = Integer.parseInt(stateNumberVal.getName().replaceAll("[^a-zA-Z0-9]", ""));
                    Fact previousAgentFact = utilityFunctions.previousRuleFact(theory, agentName.getName(), parsedStateNumber);
                    ...
                    if (previousAgentFact != null) {
                        ...
                        ensureConsistency(ruleToModify.getPreconditionFactByMatchingName("State"),
                ...
                if (postState != null) {
                    Fact newState = utilityFunctions.buildNewState(ruleToModify.getPreconditions(), preAgentState);
                ...
                if (postSnd != null) {
                    ...
                    // For each value in sentValues, remove it if it's not found in the postcondition "State"'s knowledge
                    ...
                    if (statePostFact != null && statePostFact.getParameter(2) instanceof PSpecial) {
                        ArrayList<Value> knowledge =
                                ((PSpecial) statePostFact.getParameter(2)).getGroup();
                        for (Value sVal : sentValues) {
                            if (!sVal.isConstant()) {
                                boolean found = false;
                                for (Value kn : knowledge) {
                                    if (sVal.getName().replaceAll("[^a-zA-Z]", "")
                ...
                if (ruleToModify != null && (parametersBundle.getFlags().isCombineAddReplace() && parametersBundle.getFlags().isSwitchFlag() || parametersBundle.getFlags().isCombineAddReplaceOnly())) {
