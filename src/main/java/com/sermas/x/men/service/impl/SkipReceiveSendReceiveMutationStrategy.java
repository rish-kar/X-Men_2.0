            ...
            if (originalRule.getPreconditionFactByMatchingName("Rcv") != null &&
                ...
                for (Rule candidateRule : rules) {
                    if (candidateRule.getPreconditionFactByMatchingName("Rcv") != null) {
                        ...
                        int state = Integer.parseInt(((Value) postAgentState.getParameter(1))
                ...
                for (Rule arrivingRule : mutatedTheory) {
                    ...
                    if (mutatedRule.getPostconditionFactByMatchingName("State") != null) {
                        mutatedRule.getPostconditions().set(0, utilityFunctions.buildNewState(mutatedRule.getPreconditions(), preState));
                    }
                    ...
                    for (Fact action : mutatedRule.getActions()) {
                        if (action.getF_name().equals("Snd") || action.getF_name().equals("To") ||
                                action.getF_name().equals("Rcv") || action.getF_name().equals("From")) {
                    ...
                    Fact previousStateFact = utilityFunctions.previousRuleFact(localTheoryClone, agentName.getName(), state);
                    ...
                    if (arrivingRule.getPostconditionFactByMatchingName("State") != null) {
                        arrivingRule.getPostconditions().set(0, utilityFunctions.buildNewState(originalRule.getPreconditions(), preStateCondition));
                    }

                    // Handle mutants in the receive precondition
                    ArrayList<Mutants> modifications = new ArrayList<>();
                    Object receiveParam = arrivingRule.getPreconditionFactByMatchingName("Rcv").getParameter(2);
                    ...
                    } else if (receiveParam instanceof PSpecial) {
                        PSpecial specialGroup = (PSpecial) receiveParam;
                        for (int i = 0; i < specialGroup.getGroup().size(); i++) {
                            if (specialGroup.getValue(i).isRemoved()) {
                                modifications.add(new Mutants(null, (Value) specialGroup.getValue(i)));
                            }
                        }
                    }
