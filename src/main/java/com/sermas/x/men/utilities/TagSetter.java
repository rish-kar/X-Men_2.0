...
public class TagSetter {
    ...
    public ParametersBundle setTags(ParametersBundle parametersBundle, Set<Mutations> mutationSet) {
        ...
        // Sets tags for parameters bundle depending on the mutation set (Add, Replace, Combine Add Replace, Combine Add Replace Only).
        for (Mutations mutation : mutationSet) {
            switch (mutation) {
                ...
                case Mutations.REPLACE_SUB_MESSAGES:
                    ...
                    break;
                case ADD:
                    if ((mutationSet.contains(Mutations.REPLACE_SUB_MESSAGES) || mutationSet.contains(Mutations.REPLACE_TYPE)) && mutationSet.contains(Mutations.COMBINE_ADD_REPLACE)) {
                        flags.setCombineAddReplace(true);
                        flags.setAdd(true);
                        parametersBundle.setFlags(flags);
                    }
                    if ((mutationSet.contains(Mutations.REPLACE_SUB_MESSAGES) || mutationSet.contains(Mutations.REPLACE_TYPE)) && mutationSet.contains(Mutations.COMBINE_ADD_REPLACE_ONLY)) {
