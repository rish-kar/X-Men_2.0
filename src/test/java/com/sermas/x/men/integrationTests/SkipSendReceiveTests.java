...
public class SkipSendReceiveTests {
  ...
  public void testSkipSendReceiveMultiEndpoint() throws Exception {
    ...
    // Perform the request
    mockMvc
        .perform(multipart("/api/generateMutations").file(file).header("Skip-Send-Receive", "true"))
        .andExpect(status().isOk());

    assertThat(Files.readString(Paths.get("Oyster_M0.m")).trim().replaceAll("\\r?\\n", "\n"))
        .contains(
            Files.readString(Paths.get("src/test/resources/SkipSendReceive_0.m"))
                .trim()
                .replaceAll("\\r?\\n", "\n"));
    assertThat(Files.readString(Paths.get("Oyster_M1.m")).trim().replaceAll("\\r?\\n", "\n"))
        .contains(
            Files.readString(Paths.get("src/test/resources/SkipSendReceive_1.m"))
                .trim()
                .replaceAll("\\r?\\n", "\n"));
    assertThat(Files.readString(Paths.get("Oyster_M2.m")).trim().replaceAll("\\r?\\n", "\n"))
        .contains(
            Files.readString(Paths.get("src/test/resources/SkipSendReceive_2.m"))
                .trim()
                .replaceAll("\\r?\\n", "\n"));
  }
  ...
  public void testSkipSendReceiveSingleEndpoint() throws Exception {
    ...
    // Perform the request
    mockMvc
        .perform(multipart("/api/skip/sendReceiveMutations").file(file))
        .andExpect(status().isOk());
