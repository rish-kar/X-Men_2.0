...
, SndS($Human,$GateOut,<'card','balance','gid'>,<$oyster,bal($ccard),~gid>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'card','balance','gid'>,<$oyster,bal($ccard),~gid>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
, Commit($GateOut,$Human,'finish')
]->
[ State($GateOut,'2',<~goid,$oyster,bal($ccard),~gid>)
, SndS($GateOut,$Human,<'card','balance','finish'>,<$oyster,bal($ccard),'finish'>)
]

rule H_3_M:
[ State($Human,'3',<$oyster,$ccard,bal($ccard),bal($ccard),~gid>)
, RcvS($GateOut,$Human,<'card','balance','finish'>,<$oyster,bal($ccard),'finish'>)
]
--[ H()
