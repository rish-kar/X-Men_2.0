...
, SndS($Human,$GateOut,<'balance','gid'>,<bal($oyster),~gid>)
]

rule GateOut_1_M:
[ State($GateOut,'1',<~goid>)
, RcvS($Human,$GateOut,<'balance','gid'>,<bal($oyster),~gid>)
]
--[ Receive($GateOut,$Human,$oyster)
, Receive($GateOut,$Human,bal($oyster))
, Commit($GateOut,$Human,'finish')
]->
[ State($GateOut,'2',<~goid,bal($oyster),~gid>)
, SndS($GateOut,$Human,<'balance','finish'>,<bal($oyster),'finish'>)
]

rule H_3_M:
[ State($Human,'3',<$oyster,$ccard,bal($oyster),bal($ccard),~gid>)
, RcvS($GateOut,$Human,<'balance'>,<bal($oyster)>)
]
--[ H()
